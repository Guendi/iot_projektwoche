 ...
 * rfid and pin are send to server, server grants or denies access, server message is displayed
 */
 ...
 * 4: parse server response, unlock door (for 5 seconds or so) --> back to 0
...
void loop() {
  ...
  /* handling server response */
    ...
    while(Serial.available() && nPinInput<16){
      char inKey=(char)Serial.read();
      if (inKey !='\n'){
        serverResponse[nPinInput]=inKey;
        nPinInput++;
      }
      else {
        stage=4;
...
void handleServerResponse(){
      ...
      lcd.write(emptyLine);
      lcd.setCursor(0,1);
      if (serverResponse[0]=='O' && serverResponse[1]=='K'){
